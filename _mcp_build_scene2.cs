...
public class SceneBuilder
{
    public static string Run()
    {
        try
        {
            ...
            if (controllerType == null) return "ERROR: StartMenuController type not found in Assembly-CSharp. Ensure file compiled.";

            var controllerGO = new GameObject("MenuController");
            var controller = controllerGO.AddComponent(controllerType) as MonoBehaviour;
            if (controller == null) return "ERROR: controller component null";

            // Build "开始游戏" button
            var startBtn = MakeButton(canvasGO.transform, "开始游戏", new Vector2(0, 30), new Color(0.25f, 0.55f, 0.35f), font);
            // Build "结束游戏" button
            var exitBtn = MakeButton(canvasGO.transform, "结束游戏", new Vector2(0, -140), new Color(0.70f, 0.25f, 0.25f), font);
    ...
    static GameObject MakeButton(Transform parent, string label, Vector2 pos, Color bg, Font font)
    {
