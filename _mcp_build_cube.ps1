...
$code = @'
public class T { public static string Run() { return CubeSceneBuilder.Build(); } }
'@
Write-Host '--- execute_code: CubeSceneBuilder.Build() ---'
Write-Host (Invoke-Mcp 'execute_code' @{ code = $code })

Write-Host '--- open_scene CubeScene ---'
Write-Host (Invoke-Mcp 'open_scene' @{ path = 'Assets/Scenes/CubeScene.unity' })
